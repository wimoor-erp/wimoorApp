...
const base=function(options={url:"",method:"GET",data:{},headers:{},isfile:false}){
	// var that=this
	...
	return new Promise((reslove,reject)=>{
		...
		// #endif
		if(sessionid==null&&options.url!="/admin/api/v1/auth/verifyWechatApp"&&options.url!="/admin/api/v1/auth/login"){
			 var sessionid=uni.getStorageSync("JSessionId");
			 if(sessionid){
		...
		if(options.isfile==true){
			uni.uploadFile({
						...
						success: (uploadFileRes) => {
							 if(uploadFileRes.data!=null&&uploadFileRes.data.indexOf("{")==0){
			...
			uni.request({
					...
					success:(res)=>{
						...
						}else if(res.data && res.data.code==231){
							uni.clearStorageSync();
							if(options.url!="/admin/api/v1/auth/verifyWechatApp"){
								uni.navigateTo({
