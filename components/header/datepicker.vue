...
<script setup>
	...
	const props = defineProps({
		title:"",days:0,unlimited:false,clearIcon:false,iswidth:true,dateType:'day',dateLength:7,
	 })
	...
	function getDate(type){
			   var beforedays=0;
				...
				if(props.dateLength&&!Number.isNaN(props.dateLength)&&props.dateLength===7){
					  start.setTime(start.getTime() - 3600 * 1000 * 24 * (7+beforedays))
				}else if(props.dateLength&&!Number.isNaN(props.dateLength)){
					 start.setTime(start.getTime() - 3600 * 1000 * 24 * (parseInt(props.dateLength)+beforedays))
				}else{
					 start.setTime(start.getTime() - 3600 * 1000 * 24 * (7+beforedays))
				}
	            if (type === 'start') {
