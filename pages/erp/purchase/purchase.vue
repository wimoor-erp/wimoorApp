	...
	<view>
		...
		<view class="uni-flex uni-between m-t-8">
						 <view class="uni-select-group">
							 <Warehouse  @changeWarehouse="warehouseChange"></Warehouse>
						...
						<SearchHeader ref="searchheaderRef" class="uni-flex">
						   ...
						   <uni-forms ref="baseForm" >
								<uni-forms-item label="日期范围"  >
									<Datepicker @changedate="changedate"  :clearIcon="true" :iswidth="false" :days="0"  :dateLength="365"/>
								</uni-forms-item>
		...
		<view  >
			<block v-for="(value, index) in listData" :key="value?.sku">
					<view class="uni-media-list" @click="goDetail(value)">
						...
						<view class="uni-media-list-body"  >
						  ...
						  <view class="uni-flex-item">	
							...
							<text v-if="value.issfg=='2'" class="tag-orange-plain tag-small"  @click.stop="showAssembly(value)"> 组装 </text>
							</view>
							<view class="light-font uni-right"><view class="uni-orange-data">{{value?.amount}}</view>数量</view>
							</view>
							</view>
							</view>
							...
							<view class="uni-order-footer isfull uni-between uni-flex uni-row-center">
									...
									<button  type="success" @click.stop="gotoChangeForm(value)" size="mini"  >换货</button>&nbsp;&nbsp;&nbsp;
...
<script>
	...
	export default {
		components:{Warehouse,SearchHeader,Status,Datepicker,Assembly},
		data() {
			return {
				searchType:'sku',
				search:'',
				...
				name:"",
				datetype:"createdate",
				fromDate:util.getCurrentDefineDay(0,365)[0],
				toDate:util.getCurrentDefineDay(0,365)[1],
				mydate:'',
				isOnload:true,
				status: 'more',
				contentText: {
		...
		onShow(){
			 this.ftype=uni.getStorageSync("purchase_searchtype");
			 this.search=uni.getStorageSync("purchase_search");
			 if(this.isOnload){
				 this.listData=[];
				 this.offset=1;
				 this.reload=true;
		...
		methods: {
			//展示组装信息
			showAssembly(sub){
				this.$refs["assemblyRef"].show(sub,"ass");
			},
			searchOrder(order){
				this.listData=[];
				this.offset=1;
			...
			gotoChangeForm(val){
				uni.navigateTo({
					//跳转至 换货单页面
					url:'/pages/erp/purchase/change/components/create?changeData='+ encodeURIComponent(JSON.stringify(val)),
				});
			},
			warehouseChange(val){
				this.warehouse=val;
			...
			getList() {
				...
				uni.setStorageSync("purchase_searchtype", this.ftype);
				uni.setStorageSync("purchase_search",this.search);
	            purchaseApi.list({ 
							 order:'desc',
							 sort:"createdate,number,sku",
