<template>
					...
					<view class="uni-body">
						<view class="uni-flex uni-row">
							...
							<view class="flex-item  data-mar">
								<view class='uni-num'><text>SKU数量:</text>{{quotaoderData.skuamount||quotaoderData.skunum}}</view>
								<view class="uni-num"><text>发货数量:</text>{{quotaoderData.sumQuantity||quotaoderData.sumquantity}}</view>
							</view>
						</view>
					</view>
				</uni-card>
		...
		<uni-card margin="8px 16px" class="n-shadow">
			<uni-icons type="compose" size="16"></uni-icons><text>备111注</text>
	...
	export default {
		...
		methods: {
			...
			getquotaData: function() {  
				...
				// 适应新旧版本扫码
				if(this.shipmentid.includes('FBA')){
					...
					quotaApi.getQuotainfoV2(this.shipmentid).then(data=>{
						this.quotaoderData = data.shipPlan;
						this.createdate = "创建于" + util.dateFormat(data.shipPlan.createtime)
						this.auditime = util.dateFormat(data.shipPlan.auditime) + "审核"
						this.warehouseList[0].title = data.shipPlan.warename;
						this.warehouseList[1].title = data.shipPlan.groupname + "-" + data.shipPlan.destination ;
						this.productListData = data.quatoInfo.list
						if(this.productListData){
							this.productListData.forEach(item=>{
								item.asshow=false;
							})
