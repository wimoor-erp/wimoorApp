...
function getPurchaseFormReport(data){
 ...
 function downExcelPurchaseFormReport(data,callback){
  ...
  function downloadTimeList(data,callback){
  	  return request({url:"/erp/api/v1/purchase_form/downloadTimeList",
  	                responseType:"blob",
  					params:data,
  					method:'get'}).then(res => {
  						downloadhandler.downloadSuccess(res,"PurchaseTimeList.xlsx");
  						if(callback){
  							callback();
  						}
  				}).catch(e=>{
...
export default{
	 ...
	 downExcelPurchaseFormReport,cancelPayment,getPurchaseTimeList,downloadTimeList,setPrintIp,getPrintIp,
	 getPrintIpByWarehouse,getPrintIpByAddress,
